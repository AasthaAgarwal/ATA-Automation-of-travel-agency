...
public class CustomerImpl implements Customer {
	...
	public String bookVehicle(ReservationBean reservationBean) {
		...
		if(userID!=null && vehicleID !=null && routeID!=null && bookDate!=null && journeyDate!=null && bPoint!=null && dPoint!=null && totalFare>0 )
		{
			if( userID.matches("(\\w){2}(\\d){4}") && vehicleID.matches("(\\w){2}(\\d){4}") && routeID.matches("(\\w){4}(\\d){4}")  && bPoint.length()<=30 && dPoint.length()<=30 && totalFare <=9999999999.0)
			{
